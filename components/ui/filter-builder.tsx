  ...
  return (
    <div className="relative flex-1 min-w-0" ref={ref}>
      <button type="button" onClick={() => setOpen((o) => !o)}
        className="w-full h-8 px-2.5 inline-flex items-center justify-between gap-1 rounded-lg border border-slate-200 bg-white text-xs text-slate-700 hover:border-slate-300">
      ...
      {open && (
        <div className="absolute left-0 top-9 z-50 w-64 max-h-72 overflow-hidden bg-white border border-slate-200 rounded-xl shadow-xl flex flex-col">
...
export default function FilterBuilder({ fields, value, onChange }: Props) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 top-10 z-50 w-[680px] max-w-[90vw] max-h-[70vh] overflow-y-auto bg-white border border-slate-200 rounded-xl shadow-2xl p-4">
                    ...
                    return (
                      <div key={cond.id}>
                        ...
                        <div className="flex items-center gap-2">
                          <StyledSelect value={cond.field} onChange={(e) => onFieldChange(group.id, cond.id, e.target.value)} className="w-44 h-8 shrink-0">
                            <option value="">Select field…</option>
                            {fields.map((f) => <option key={f.key} value={f.key}>{f.label}</option>)}
                          </StyledSelect>
                          {field && (
                            <StyledSelect value={cond.operator} onChange={(e) => patchCondition(group.id, cond.id, { operator: e.target.value, value: "" })} className="w-40 h-8 shrink-0">
                          ...
                          {field && op && !op.noValue && (
                            op.multi && field.type === "select" ? (
                              ...
                              <input type="number" value={cond.value as string} onChange={(e) => patchCondition(group.id, cond.id, { value: e.target.value })}
                                placeholder="Value" className="flex-1 min-w-0 h-8 px-2.5 rounded-lg border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            ) : field.type === "date" ? (
                              <input type="date" value={cond.value as string} onChange={(e) => patchCondition(group.id, cond.id, { value: e.target.value })}
                                className="flex-1 min-w-0 h-8 px-2.5 rounded-lg border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            ) : (
                          ...
                          <button type="button" onClick={() => removeCondition(group.id, cond.id)}
                            className="h-7 w-7 shrink-0 inline-flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-200 hover:text-slate-700">
