    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex gap-1 bg-slate-100 rounded-lg p-1">
          <button
            onClick={() => { setTab("practices"); setSearch("") }}
            className={cn("px-4 py-1.5 text-sm font-medium rounded-md transition-colors", tab === "practices" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700")}
          >
            <Building2 className="h-3.5 w-3.5 inline mr-1.5 -mt-0.5" />
            Practices
            <span className="ml-1.5 text-xs text-slate-400">{practices.length}</span>
          </button>
          <button
            onClick={() => { setTab("providers"); setSearch("") }}
            className={cn("px-4 py-1.5 text-sm font-medium rounded-md transition-colors", tab === "providers" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700")}
          >
            <User className="h-3.5 w-3.5 inline mr-1.5 -mt-0.5" />
            Providers
            <span className="ml-1.5 text-xs text-slate-400">{allProviders.length}</span>
          </button>
        </div>
