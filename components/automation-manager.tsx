...
function MultiSelectValue({ options, value, onChange }: {
  ...
  return (
    <div ref={ref} className="relative flex-1 min-w-[160px]">
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full max-h-60 overflow-hidden bg-white border border-slate-200 rounded-md shadow-lg flex flex-col">
          {options.length > 6 && (
            <div className="border-b border-slate-100 p-1.5">
              <input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Search…"
                className="w-full px-2 py-1 text-sm rounded border border-slate-200 focus:outline-none focus:border-slate-400" />
            </div>
          )}
          <div className="overflow-y-auto py-1">
            {filtered.length === 0 && <p className="px-3 py-2 text-xs text-slate-400">No options</p>}
            {filtered.map(o => (
