...
async function loadCustomObjectCards(objectType: string, record: Record<string, any>, ownerLabel: string) {
  ...
  const catalog: RecordFieldDef[] = [
    ...props.map((p) => ({ key: p.id, label: p.name, type: CP_TYPE[p.type] ?? "text", options: p.options ?? [], optionLabels: (p as any).optionLabels ?? undefined, optionColors: (p as any).optionColors ?? undefined, optionStyle: (p as any).optionStyle ?? undefined, visibilityRule: (p as any).visibilityRule ?? undefined, numberFormat: (p as any).numberFormat ?? undefined })),
...
export async function loadPropertyCards(entityType: string, record: Record<string, any>, ownerLabel?: string) {
  ...
  const catalog: RecordFieldDef[] = [
    ...
    ...customProps.map((c) => ({ key: `cp_${c.id}`, label: c.name, type: CP_TYPE[c.type] ?? "text", options: c.options, default: (c as any).defaultValue ?? undefined, conditional: (c as any).conditional ?? undefined, optionLabels: (c as any).optionLabels ?? undefined, optionColors: (c as any).optionColors ?? undefined, optionStyle: (c as any).optionStyle ?? undefined, visibilityRule: (c as any).visibilityRule ?? undefined, numberFormat: (c as any).numberFormat ?? undefined })),
