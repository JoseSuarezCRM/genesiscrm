...
export async function loadPropertyCards(entityType: string, record: Record<string, any>, ownerLabel?: string) {
  ...
  const catalog: RecordFieldDef[] = [
    ...
    ...customProps.map((c) => ({ key: `cp_${c.id}`, label: c.name, type: CP_TYPE[c.type] ?? "text", options: c.options, default: (c as any).defaultValue ?? undefined, conditional: (c as any).conditional ?? undefined })),
